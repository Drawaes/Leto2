...
{
    public class EphemeralOwnedMemory : OwnedMemory<byte>
    {
        ...
        public EphemeralOwnedMemory(int offset, int length, EphemeralBufferPool pool)
        {
            _pool = pool;
            _offset = offset;
            _length = length;
        ...
        private IntPtr PoolPointer
        {
            get
            {
                if (_disposed)
                {
                    throw new ObjectDisposedException(nameof(EphemeralOwnedMemory));
                }

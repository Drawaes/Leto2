...
namespace Leto.OpenSsl11
{
    public sealed class OpenSslECCurveKeyExchange : IKeyExchange
    {
        ...
        public unsafe void PublicKeySpan(Span<byte> span)
        {
            GenerateKeyPair();
            ...
            var size = EC_POINT_point2oct(group, pubKey, EC_POINT_CONVERSION.POINT_CONVERSION_UNCOMPRESSED, span);
        }
        ...
        private void GenerateKeyPair()
        {
            ...
            EVP_PKEY_paramgen_ECCurve(_curveNid, out var curveParameters);
            try
            {
        ...
        {
            writer.WriteBigEndian((byte)(ClientSendSize - 1));
            Span<byte> span = stackalloc byte[ClientSendSize - 1];
            PublicKeySpan(span);
            writer.Write(span);
        }

        public int ClientSendSize => _keyExchangeSize + 1;

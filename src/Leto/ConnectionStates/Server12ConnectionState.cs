...
namespace Leto.ConnectionStates
{
    public sealed partial class Server12ConnectionState : ConnectionState, IConnectionState
    {
        ...
        private void WriteCertificates() => this.WriteHandshakeFrame(3 + CertificateWriter.GetCertificatesSize(_certificate), (ref WriterWrapper buffer) =>
                 CertificateWriter.WriteCertificates(ref buffer, _certificate), HandshakeType.certificate);
        ...
        public override bool ProcessHandshake()
        {
            if (!_initialSendDone)
            {
                //Server doesn't send anything first
